...
def user_meal_course_objs(meal_objs, meal_course_objs):
    ...
    UserMealStatus.objects.bulk_create([
        UserMealStatus(
            ...
            custom_meal_quantity=user_meal['custom_meal_quantity']
        )
...
def items_and_rating_dtos(item_objs, meal_objs):
    items_and_rating_dtos_list = [
        ItemAndRatingDto(item_id=1, quality=3, taste=4),
        ItemAndRatingDto(item_id=2, quality=3, taste=4),
...
@pytest.fixture
def user_rating_dto(items_and_rating_dtos, user_rating_objs, user_feedback):
